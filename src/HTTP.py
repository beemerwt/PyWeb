...
class Response:
    ...
    def get(self):
        fetched = FileManager.FetchedFile(self.path)
        general_header = General()
        response_header = HResponse(etag=fetched.checksum)
        ...
        if self.status_code == 404:
            ...
            entity_header.c_type = fetched.type
            entity_header.c_length = fetched.size
            entity_header.c_encoding = "gzip"
            response_header.retry = 120
            message_body += fetched.encoded
            message_body += CRLF
