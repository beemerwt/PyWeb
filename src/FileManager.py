...
from os.path import getsize, isfile, relpath, splitext, isdir
...
class FetchedFile:
    def __init__(self, path):
        ...
        self.original = open(self.path)
        if self.original is None: return  # Call "open" for delegate functionality.

        self.original = self.original.read()
...
def get_safe(path):
    ...
    if not isdir(path) and isfile(path): return path[1:]
...
def allowed_access(client, _file):
